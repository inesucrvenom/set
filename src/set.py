...
set30 = simplegui.load_image(
    "https://raw.githubusercontent.com/inesucrvenom/set/master/assets/set30.png"
)
set31 = simplegui.load_image(
    "https://raw.githubusercontent.com/inesucrvenom/set/master/assets/set31.png"
)
    ...
    for i in range(0, 5):
        for j in range(0, 5):
            if board[i][j][-1] == 0:  # is selected = last item in list
                canvas.draw_image(
                    set30, pos_set(board[i][j]), dim, pos(i, j), dim
                )
            else:
                canvas.draw_image(
                    set31, pos_set(board[i][j]), dim, pos(i, j), dim
                )
